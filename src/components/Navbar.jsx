...
const Navbar = () => {
  return (
    ...
    boxShadow: '0 4px 8px'}}>
      ...
      <NavLink to="/chirps" style={({ isActive }) => ({ color: isActive ? 'black' : 'blue', boxShadow: isActive ? '0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)': 'none', background : isActive ? 'lightgrey' : 'none', padding: isActive ? '5px' : '5px', margin: isActive ? '5px' : '5px',  borderRadius: isActive ? '5px' : 'none', textDecoration: isActive? 'none' : 'none' })} className={({ isActive }) => `nav_link${isActive ? " active" : ""}`} >Chirps</NavLink>
